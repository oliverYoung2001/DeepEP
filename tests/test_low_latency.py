...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,    # num_experts=288=256+32, num_topk=8
              rank: int, num_ranks: int, group: dist.ProcessGroup, buffer: deep_ep.Buffer, seed: int = 0):
    if rank == 0:
        print(f'[config] num_tokens={num_tokens}, hidden={hidden}, num_experts={num_experts}, num_topk={num_topk}', flush=True)
    ...
    topk_idx = torch.topk(scores, num_topk, dim=-1, largest=True, sorted=True)[1]   # [NT, TopK]
    ...
    avg_t, min_t, max_t = bench(partial(test_func, zero_copy=False, return_recv_hook=False), num_warmups=WARMUP, num_tests=TIMES)
    # avg_t, min_t, max_t, ts = bench(partial(test_func, zero_copy=False, return_recv_hook=False), num_warmups=WARMUP, num_tests=TIMES)
    # if rank == 0:
    if True:
    ...
    for return_recv_hook in (False, True):
        ...
        dispatch_t, combine_t = bench_kineto(partial(test_func, zero_copy=True, return_recv_hook=return_recv_hook),
                                             ...
                                             suppress_kineto_output=True, num_tests=TIMES)
...
def test_loop():
    ...
    num_rdma_bytes = deep_ep.Buffer.get_low_latency_rdma_size_hint(max(NTs), hidden, num_ranks, num_experts)
    # if local_rank == 0:
    if rank == 0:
        print(f'Allocating buffer size: {num_rdma_bytes / 1e6} MB ...', flush=True)
    buffer = deep_ep.Buffer(group, num_rdma_bytes=num_rdma_bytes, low_latency_mode=True,
                            num_qps_per_rank=num_experts // num_ranks)
    for num_tokens in NTs:
